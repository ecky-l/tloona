...
snit::widget ::Tmw::Console {
    ...
    constructor {args} {
        ...
        #bind $textwin <KeyPress> [list puts %M,%K]
        ...
        #$self configure -relief flat -background white
    ...
    method eval {cmd} {
        ...
        if {[catch {uplevel #0 $cmd} result]} {
            append result \n
            $textwin fastinsert insert $result error
        } else {
            if {$result != ""} {
                append result \n
            }
            $textwin fastinsert insert $result
        }
        if {[lindex $cmd 0] == "cd"} {
            # update prompt
    ...
    method evalExtern {cmd} {
        ...
        set options(-history) [linsert $options(-history) \
            0 [string trimright $cmd "\n"]]
    ...
    method setAliases {} {
        ...
        interp alias {} puts {} ::Tmw::Console::putsAlias $self
        interp alias {} exit {} ::Tmw::Console::exitAlias $self
        interp alias {} gets {} ::Tmw::Console::getsAlias $self
    }
...
snit::widgetadaptor ::Tmw::SlaveConsole {
    delegate method * to hull
    delegate option * to hull
    
    constructor {args} {
        installhull using Console
        $self configurelist $args
    }
}
